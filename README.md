...
```bash
OPENAI_API_KEY=sk-... npm run match:voice -- --dry-run   # 계획만 확인 (권장: 먼저 이걸로)
OPENAI_API_KEY=sk-... npm run match:voice                # 실제 반영
```

판별 순서는 ① 파일 이름이 대본의 이름과 맞으면 그대로 ② 음성 인식 후 600개 대사와 글자 단위 유사도
비교 ③ `--order`(인식 없이 *대본 페이지에 보이는 순서* = 녹음 순서로 1:1 배치)입니다.

- 옮기기 전에 앞뒤 무음·끝 클릭을 다듬고 m4a(AAC 모노 44.1kHz)로 변환한 뒤
  `recorded-assets.json`·`manifest.json`까지 갱신합니다. 앱을 새로 고치면 바로 그 육성이 나옵니다.
- 유사도가 낮거나(`--min`, 기본 0.62) 같은 대사에 파일이 여러 개면 **보류**로 표시하고 건드리지
  않습니다. 이미 녹음이 있는 대사도 건너뜁니다(덮어쓰려면 `--rerecord`).
...
- 이미 전사 텍스트가 있으면(음성 메모 앱의 받아쓰기 등) API 없이 쓸 수 있습니다:
